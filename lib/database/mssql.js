...
import SQL_QUERY from "./sqlquery.js";

const Singleton = (function () {
  let pool;
  async function createPool(conf) {
    try{
      return await db.connect(conf);
    } catch (err) {
      console.log('Error - connection,', err);
    }
  }
  return {
    getPool: async function (conf) {
      if (!pool) {
        pool = await createPool(conf);
...
export default function (config, ctx_name = 'default') {
  ...
  const initCfgData = async () => {
    ...
    const { recordset } = await pool.request().query(SQL_QUERY.endpoint);
